...
def note_shift(note, instrument):
    audio_file = f'resources/data/{instrument}/c_note.mp3'  # путь к аудиофайлу
    ...
    # сохранение файла
    sf.write('resources/note.mp3', y, samplerate=sr)


# получение интервала
def interval_shift(interval, root, instrument):
    audio_file = f'resources/data/{instrument}/{INTERVALS_TO_FILES[interval]}.mp3'
    ...
    sf.write('resources/interval.mp3', y, samplerate=sr)

# получение гаммы
def scale_shift(scale, key, instrument):
    audio_file = f'resources/data/{instrument}/{SCALES_TO_FILES[scale]}.mp3'
    y, sr = librosa.load(audio_file)
    ...
    sf.write('resources/scale.mp3', y, samplerate=sr)

# получение аккорда
def chord_shift(chord, root, instrument, add=''):
    audio_file = f'resources/data/{instrument}/{CHORDS_TO_FILES[chord]}.mp3'
    if add != 'None':
        # поиск файла аккорда с указанной дополнительной ступенью
        audio_file = f'resources/data/{instrument}/{CHORDS_TO_FILES[chord]}_{add}.mp3'
    ...
    sf.write('resources/chord.mp3', y, samplerate=sr)
